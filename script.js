...
//         // If permission is 'prompt' or 'denied', try to get the position,
//         // which will trigger the permission request.
...
//         // If permission is already granted, just get the current position
...
//   try {
//     const coords = await fetchUserLocation();
//     if (coords) { // Ensure coords are not undefined if permission was denied
//       console.log('value of checkLocation.....', checkLocation);
//       lat = coords.lat;
//       lon = coords.lon;
//       console.log('for getWeather function in getLocation function lat=', lat, ' lon=', lon);
//       console.log('getWeather function calling..');
//       await getWeather(lat, lon);
//       console.log('getWeather function called..');
//     } else {
//       errordisplay.innerHTML = 'Location access denied or unavailable.';
//       showErrorDisplay();
...
//     errordisplay.innerHTML = ' Please allow location access.';
//     showErrorDisplay();
//     console.log('location not allowed...', e); // Log the error for debugging
//   }
// }
...
//   async function getCurrentCoordinates() {
//     return new Promise((resolve, reject) => {
//       // You can add options here, e.g., enableHighAccuracy, timeout
//       navigator.geolocation.getCurrentPosition(
//         position => resolve(position),
//         error => reject(error),
//         { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 } // Added options for better accuracy/timeout
//       );
//     });
//   }
...
//         // If 'prompt', this call will trigger the prompt.
//         // If 'denied', this call will immediately fail with PERMISSION_DENIED.
//         console.log("Attempting to request location access...");
//         const position = await getCurrentCoordinates();
//         lat = position.coords.latitude;
//         lon = position.coords.longitude;
//         console.log('inside fetchUserLocation function lat=', lat, ' lon=', lon);
...
//       switch (error.code) {
//         case error.PERMISSION_DENIED:
//           errorMessage = 'Permission denied. Please allow location access in your browser or device settings. **Make sure your site is using HTTPS.**';
//           break;
//         case error.POSITION_UNAVAILABLE:
//           errorMessage = 'Location information is unavailable. This might be due to GPS issues or network problems.';
//           break;
//         case error.TIMEOUT:
//           errorMessage = 'The request to get your location timed out. Please ensure good GPS/network signal.';
//           break;
//         default:
//           errorMessage = `An unknown error occurred: ${error.message || error.code}.`;
//           break;
//       }
//       errordisplay.innerHTML = errorMessage;
//       console.error('Geolocation Error:', error); // Log the full error for debugging
//       return null; // Return null or throw an error to propagate
//     }
//   }
...
//       console.log('getWeather function called..');
//     } else {
//       // This block will be hit if fetchUserLocation returns null (e.g., on permission denied)
//       errordisplay.innerHTML = 'Could not get your location. Please ensure location access is enabled for this website.';
//       showErrorDisplay();
//       console.log('Location access not obtained.');
//     }
//   } catch (e) {
//     errordisplay.innerHTML = 'An unexpected error occurred while trying to get your location.';
//     showErrorDisplay();
//     console.error('Unhandled error in getLocation:', e);
//   }
// }

// Ensure showErrorDisplay and getWeather functions are defined elsewhere in your code
// For testing purposes:
// function showErrorDisplay() { console.error('Error Displayed'); }
// async function getWeather(lat, lon) { console.log(`Fetching weather for ${lat}, ${lon}`); return true; }
